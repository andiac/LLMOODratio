...
MODELS = {
    ...
    "SVM": (SVC(kernel="sigmoid", gamma=1.0, probability=True), 3000),
...
def predict_proba(trainer, model, dataset, tokenizer=None):
    """Convert the predict_proba function to specific classes to unify the API."""
    if type(model).__name__ == "SetFitModel":
        return model(dataset["text"])
        ...
        # Split the dataset into multiple datasets, one for each row
        split_datasets = [dataset.select([i]) for i in range(len(dataset))]
        predictions = []
        
        # print(tokenizer("spam ham spam spam spam spam", padding="longest", max_length=512, truncation=True, return_tensors="pt"))
        for data in dataset:
            # https://www.kaggle.com/code/tigerh/learning-llm-and-using-flan-t5-logits
            generateOutput = model.generate(**tokenizer(data["text"], padding="longest", max_length=512, truncation=True, return_tensors="pt").to(model.device), output_scores=True, return_dict_in_generate=True)
            # print(len(generateOutput["scores"]))
            # print(torch.argmax(generateOutput["scores"][0]), torch.argmax(generateOutput["scores"][1]), torch.argmax(generateOutput["scores"][2])) 
            # print(generateOutput["scores"][1].shape)
            # if len(generateOutput["scores"]) >= 2:
            cur_prediction = []
            for i in range(len(generateOutput["scores"])):
                # 13655 is the token of "spam"
                cur_prediction.append(F.log_softmax(generateOutput["scores"][i], dim=1)[0, 13655].item())
            predictions.append(max(cur_prediction))
            # else:
        ...
        return F.log_softmax(torch.tensor(trainer.predict(dataset).predictions), dim=1)[:, 1]
                ...
                for model_name, (model, max_iter) in MODELS.items():
                    ...
                    if test_set == "test":
                        ...
                        for score_name, score_fn in SCORING.items():
                            if score_name in ["auroc", "aupr", "fpr95"]:
                                scores.loc[model_name][score_name] = score_fn(
                                    model.predict_proba(X_test)[:, 1], y_test)
                            else:
                                scores.loc[model_name][score_name] = score_fn(
                                    y_test, y_pred
                                )
