...
class DatasetUtil():
    ...
    def get_dataset(self, dataset_name, split):
        ...
        if dataset_name == 'news-category-hf':
            dataset = load_dataset('heegyu/news-category-dataset', split=split, cache_dir=self.hf_cache_dir)
            processed_dataset = dataset['train'].train_test_split(test_size=17968)
            return processed_dataset
        ...
        if dataset_name == 'clinc150':
            raw_data = json.load(open(os.path.join(exp_configs.DATA_DIR, dataset_name, 'data_full.json'), 'r'))
            ...
            json.dump(labels_to_label_indices, open(os.path.join(exp_configs.DATA_DIR, dataset_name, 'labels_to_label_indices.json'), "w"), indent = 4)
            ...
            processed_dataset.save_to_disk(os.path.join(exp_configs.DATA_DIR, dataset_name, 'clinc150_processed'))
